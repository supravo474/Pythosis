...
def main(root, text, menubar):
    ...
    for value in range(1, 73):
        ssubmenu.add_command(label=str(value), command=lambda value=value: font.configure(size=value))

    formatMenu.add_command(label="Switch to Dark Mode", command=objFormat.changeBg)
    ...
    formatMenu.add_command(label="Switch to Default/Light Mode", command=objFormat.default)
